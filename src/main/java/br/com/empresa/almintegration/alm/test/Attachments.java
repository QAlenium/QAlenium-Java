...
public class Attachments {
	...
	public static void main(String[] args) throws Exception {
		...
		new Attachments().attachmentsExample(
				"http://" + c.HOST + ":" +
						c.PORT + "/qcbin",
						c.DOMAIN,
						c.PROJECT,
  ...
  View that code to learn more on updating.
		...
		//delete attachments
		writeExample.deleteEntity(newOctetStreamAttachmentUrl, false);
		writeExample.deleteEntity(newMultiPartAttachmentUrl, false);
		...
		writeExample.deleteEntity(createdEntityUrl, false);
		login.logout();
	}
   ...
   An alt query parameter could also have been used. */
		...
		Response readResponse =
				con.httpGet(attachmentUrl, null, requestHeaders, false);
   ...
   An alt query parameter could also have been used. */
	...
	private String readAttachments(String entityUrl) throws Exception {
		...
		Response readResponse =
				con.httpGet(entityUrl + "/attachments", null, requestHeaders, false);

		if (readResponse.getStatusCode() != HttpURLConnection.HTTP_OK) {
			throw new Exception(readResponse.toString());
		}

		return readResponse.toString().replaceAll("&lt;", "").replaceAll("html&gt;", "").replaceAll("body&gt;", "").replaceAll("//", "");
	}
	...
	public String attachWithOctetStream(String entityUrl, byte[] fileData,
		...
		Map<String, String> requestHeaders = new HashMap<String, String>();
		requestHeaders.put("Slug", filename.split("/")[7]);
		requestHeaders.put("Content-Type", "application/octet-stream");

		Response response =
				con.httpPost(entityUrl + "/attachments", fileData, requestHeaders, true);
