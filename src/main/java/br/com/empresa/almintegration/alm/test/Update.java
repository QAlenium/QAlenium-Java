...
public class Update {
	...
	public void updateExample(final String serverUrl, final String domain,
		...
		//cleanup
		writeExample.deleteEntity(newEntityToUpdateUrl, false);
		login.logout();
	...
	public String checkout(String entityUrl, String comment, int version)
		...
		Response response =
				con.httpPost(entityUrl + "/versions/check-out", xml.getBytes(), requestHeaders, true);
	...
	public String lock(String entityUrl) throws Exception {
		...
		if (lockResponse.getStatusCode() < HttpURLConnection.HTTP_OK || 
				lockResponse.getStatusCode() > HttpURLConnection.HTTP_MOVED_TEMP) {
	...
	public boolean unlock(String entityUrl) throws Exception {
		...
		return con.httpDelete(entityUrl + "/lock", null, false).getStatusCode() == HttpURLConnection.HTTP_OK;
	}
	...
	private Response update(String entityUrl, String updatedEntityXml)
		...
		Response putResponse =
				con.httpPut(entityUrl, updatedEntityXml.getBytes(), requestHeaders, false);
